...
public class Search_Runner {
    ...
    public Search_Runner(int [][] matrix,int n) {
        OpenList=new Hashtable<>(100000000);
        CloseList=new Hashtable<>(100000000);
        priorityQueue=new PriorityQueue(100000000, new MyComparator());
        initialState=new State(matrix, n);
        finalState=new State(n);
        //finalState.printstate();
        this.squaresize=n;
        MaxNodeExpanded=100000000;
...
//        System.out.println("Initial Heuristics Value: "+ init_h_cost);
        ...
        CloseList.clear();
        HashingSet.clear();
        OpenList.put(initialState, init_h_cost);
...
//                }
                ...
                if(OpenList.containsKey(childState))
                {
                    int val=OpenList.get(childState);
                    if(val<(current.g_cost)+1+temp_h_cost)
                    {
                        continue;
                    }
                }
